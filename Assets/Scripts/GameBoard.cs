...
public class GameBoard
{
  ...
  //  public Gem[,] AllGems { get { return allGems; } }
    ...
    private List<SC_Gem> currentBombMatches = new List<SC_Gem>();
    public List<SC_Gem> CurrentBombMatches { get { return currentBombMatches; } }
    #endregion
            ...
            for (int y = bombPos.y - _BlastSize; y <= bombPos.y + _BlastSize; y++)
            {
                if (x >= 0 && x < width && y >= 0 && y < height)
                {
                    if (allGems[x, y] != null)
                    {
                        ...
                        currentBombMatches.Add(allGems[x, y]);
                    }
                }
            }
        }
        currentBombMatches = currentBombMatches.Distinct().ToList();
    }
}
